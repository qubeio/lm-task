/**
 * tools/utils.js
 * Utility functions for LMTasker CLI integration
 */
...
function getProjectRoot(projectRootRaw, log) {
  ...
  // 4. Check if the current directory has any indicators of being a LMTasker project
  ...
  log.warn(
    `No LMTasker project detected in current directory. Using ${currentDir} as project root.`,
  );
  log.warn(
  ...
  try {
    ...
    if (serverPath) {
      ...
      if (serverPath.includes("mcp-server")) {
        ...
        // Verify this is a LMTasker project by checking for specific markers
        ...
        if (hasLMTaskerMarkers) {
          log.info(
            `Using project root derived from MCP server path: ${potentialProjectRoot}`,
          );
...
/**
 * Executes a LMTasker CLI command synchronously.
 ...
 */
function executeLMTaskerCommand(
  command,
  log,
  args = [],
  projectRootRaw = null,
  ...
  try {
    ...
    log.info(
      `Executing lmtasker ${command} with args: ${JSON.stringify(
        args,
      )} in directory: ${cwd}`,
    );
    ...
    // Execute the command using the global lmtasker CLI or local script
    // Try the global CLI first
    let result = spawnSync("lmtasker", fullArgs, spawnOptions);

    // If global CLI is not available, try fallback to the local script
    if (result.error && result.error.code === "ENOENT") {
      log.info("Global lmtasker not found, falling back to local script");
    ...
    log.error(`Error executing lmtasker command: ${error.message}`);
    return {
      success: false,
      error: error.message,
    };
