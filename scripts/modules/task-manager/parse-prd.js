...
async function parsePRD(prdPath, tasksPath, numTasks, options = {}) {
  ...
  if (isOldFormatPRD(prdPath) || isInScriptsDirectory(prdPath)) {
    displayDeprecationWarning(prdPath, outputFormat, logFn);

    // For CLI users, offer migration prompt (non-blocking)
    if (outputFormat === "text" && !isSilentMode()) {
      await promptForMigration(prdPath, projectRoot);
    }
  }
    ...
    Return your response in this format:
{
    ...
    // Ensure JSON output file exists for MCP/JSON mode (tests expect real file)
    try {
      fs.writeFileSync(tasksPath, JSON.stringify(outputData, null, 2));
    } catch (_err) {
      // ignore write errors here, as writeJSON handles primary writing
    }
    report(
      ...
      console.log(
        boxen(
          chalk.white.bold("Next Steps:") +
            ...
            `${chalk.cyan("2.")} Run ${chalk.yellow('task-master add-subtask --parent=<id> --title="Subtask title"')} to break down a task into subtasks`,
          {
            padding: 1,
            borderColor: "cyan",
            borderStyle: "round",
