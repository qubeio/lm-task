...
async function addTask(
	...
	try {
		...
		if (manualTaskData) {
			...
			// Add a visual transition to show we're moving to AI generation - only for text mode
			if (outputFormat === 'text') {
				console.log(
					boxen(
			...
			console.log(
				boxen(
					chalk.white.bold(`Task ${newTaskId} Created Successfully`) +
						...
						'\n' +
						chalk.cyan(
							`3. Run ${chalk.yellow(`task-master add-subtask --parent=${newTaskId} --title="Subtask title"`)} to break it down into subtasks`
						),
