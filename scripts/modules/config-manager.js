...
// Research functions removed - Azure-only deployment doesn't support research role
...
// Ollama functions removed - Azure-only deployment doesn't support Ollama

/**
 * Gets the Azure OpenAI base URL from the global configuration.
 * @param {string|null} explicitRoot - Optional explicit path to the project root.
 * @returns {string|undefined} The Azure OpenAI base URL or undefined if not found.
 */
function getAzureOpenaiBaseUrl(explicitRoot = null) {
	// Directly return value from config
	return getGlobalConfig(explicitRoot).azureOpenaiBaseUrl;
}
...
function isApiKeySet(providerName, session = null, projectRoot = null) {
	...
	if (providerKey !== 'azure') {
		log(
			'warn',
			`Unsupported provider: ${providerName}. Only Azure is supported in this deployment.`
		);
		return false;
	}

	const apiKeyValue = resolveEnvVariable(
...
function getMcpApiKeyStatus(providerName, projectRoot = null) {
	...
	if (providerName !== 'azure') {
		console.warn(
			chalk.yellow(
				`Warning: Unsupported provider ${providerName}. Only Azure is supported.`
			)
		);
		return false;
	}
	...
	try {
		...
		const apiKeyToCheck = mcpEnv.AZURE_OPENAI_API_KEY;
...
function getBaseUrlForRole(role, explicitRoot = null) {
	...
	// Check for role-specific baseUrl first
	if (roleConfig && typeof roleConfig.baseUrl === 'string') {
		return roleConfig.baseUrl;
	}

	// For Azure provider, use the global azureOpenaiBaseUrl
	if (roleConfig && roleConfig.provider === 'azure') {
		return getAzureOpenaiBaseUrl(explicitRoot);
	}
...
export {
	...
	// Role-specific getters (Azure-only)
	getMainProvider,
	getMainModelId,
	getMainMaxTokens,
	getMainTemperature,
