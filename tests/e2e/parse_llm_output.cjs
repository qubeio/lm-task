...
rl.on("close", async () => {
  ...
  try {
    ...
    // Support both Anthropic and Azure OpenAI response formats
    let textContent = apiResponse?.content?.[0]?.text; // Anthropic format
    if (!textContent) {
      textContent = apiResponse?.choices?.[0]?.message?.content; // Azure OpenAI format
    }
    if (!textContent) {
      console.error(
        chalk.red(
          "Error: Could not find response content in either '.content[0].text' (Anthropic) or '.choices[0].message.content' (Azure OpenAI) format.",
        ),
      );
      process.exit(1);
    }
