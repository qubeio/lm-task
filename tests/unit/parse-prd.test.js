...
describe("parse-prd extension compatibility", () => {
  ...
  test.each(["simple-prd.md", "legacy-prd.txt"])("handles %s correctly", async (file) => {
    const prd = path.join(fixtures, file);
    const out = path.join(output, `${file}.json`);
    const res = await parsePRD(prd, out, 1, { force: true, mcpLog: { info: jest.fn(), warn: jest.fn(), error: jest.fn(), debug: jest.fn(), success: jest.fn() } });
  ...
  test("calls AI generation for both extensions", async () => {
    ...
    await parsePRD(path.join(fixtures, "simple-prd.md"), path.join(output, "md.json"), 1, { force: true, mcpLog: logger });
    await parsePRD(path.join(fixtures, "legacy-prd.txt"), path.join(output, "txt.json"), 1, { force: true, mcpLog: logger });
    ...
    test("treats different extensions the same way", async () => {
      ...
      expect(typeof mdResult.telemetryData).toBe(
        typeof txtResult.telemetryData
      );
    });
...
expect(savedTasks.tasks).toBeDefined();
  ...
  describe("Enhanced Error Handling", () => {
    ...
    test("handles AI service failure gracefully", async () => {
      mockGenerateObjectService.mockRejectedValueOnce(new Error("AI service unavailable"));
      const prdPath = path.join(testFixturesDir, "simple-prd.md");
