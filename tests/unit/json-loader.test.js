...
const { JsonLoader } = await import(
  "../../scripts/modules/tmui/src/utils/jsonLoader.js"
);

describe("JsonLoader", () => {
  let jsonLoader;
  const mockProjectRoot = "/mock/project/root";
  const mockTasksFile = path.default.join(
    mockProjectRoot,
    "tasks",
    "tasks.json",
  );
  ...
  describe("calculateStats", () => {
    ...
    test("should return empty stats when tasks array is null or empty", () => {
      expect(jsonLoader.calculateStats(null)).toEqual(
        jsonLoader.getEmptyStats(),
      );
